...
export async function start(): Promise<void> {
  ...
  const tsxBin = isWindows ? "npx" : path.resolve(ROOT, "node_modules", ".bin", "tsx");
  const spawnArgs = isWindows ? ["tsx", sellerScript] : [sellerScript];

  ensureLogsDir();
  const logFd = fs.openSync(SELLER_LOG_PATH, "a");

  const sellerProcess = spawn(tsxBin, spawnArgs, {
    detached: true,
    shell: isWindows,
